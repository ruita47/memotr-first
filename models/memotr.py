...
from .kalman_filter import build_kalman_filter
...
class MeMOTR(nn.Module):
    def __init__(self, backbone: BackboneWithPE, transformer: DeformableTransformer,
        ...
        # KPRR: Initialize Kalman-Prompted Residual Refinement
        if self.use_kprr and self.kalman_filter is not None:
            self.query_updater.kalman_filter = self.kalman_filter
...
def build(config: dict):
    ...
    # KPRR: Build Kalman Filter if enabled
    use_kprr = config.get("USE_KPRR", False)
    kalman_filter = None
    if use_kprr:
        kalman_filter = build_kalman_filter(config=config)
