    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                # KPRR Stage 2: Non-linear Residual Refinement
                # Instead of directly adding tmp to inverse_sigmoid(reference_points),
                # we treat tmp as the residual Delta_P_res and apply sigmoid safely
                if reference_points.shape[-1] == 4:
                    # P_final = sigma(P_ref + Delta_P_res)
                    # tmp is already the residual from MLP, reference_points is P_ref
